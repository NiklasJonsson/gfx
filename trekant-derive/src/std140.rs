...
fn derive_std140_compat(di: &DeriveInput) -> Result<TokenStream, CompileError> {
    ...
    let Acc {
        ...
        let max_alignment = quote! {
            trekant::util::max(#max_alignment, <#ty as #trait_name>::ALIGNMENT)
        };
    ...
    Ok(quote! {
        unsafe impl #impl_generics #trait_name for #name #ty_generics #where_clause {
            ...
            const SIZE: usize = trekant::util::round_to_multiple(#offset_expr, Self::ALIGNMENT);
            const ALIGNMENT: usize = trekant::util::round_to_multiple(#max_alignment, 16);
        }
