...
impl VKManager {
    ...
    fn send_data_to_gpu(&self, primitive: &Primitive) -> Renderable {
        ...
        let g_pipeline = match primitive.vertex_data {
            VertexBuf::Base(_) => {
                pipeline::create_graphics_pipeline::<VertexBase>(
                    &self.vk_device,
                    &self.render_pass,
            ...
            VertexBuf::UV(_) => {
                pipeline::create_graphics_pipeline::<VertexUV>(
                    &self.vk_device,
                    &self.render_pass,
                    self.swapchain.dimensions(),
                    mode,
        ...
        submit_material_uniform_data(&self.vk_device, &self.graphics_queue, &primitive.material);
    ...
    pub fn draw_next_frame(&mut self, world: &mut World) {
        ...
        let builder = renderables.join().fold(builder, |builder, renderable| {
            renderable.record_draw_commands(builder, &transforms_sub_buf,
                                   &lighting_sub_buf, &model_ubo_buf)
        });
...
fn submit_material_uniform_data(
    ...
    if let Material::GlTFPBRMaterial{
        ...
        let (buf, mat_copied) = ImmutableBuffer::from_data(
            data,
        ...
        let (tex_access, fut) = match base_color_texture {
            Some(texture) => {
                ...
                (Some(TextureAccess{image_buffer, sampler}),
            ...
            match &self.base_color_tex {
                Some(tex_access) => {
                    Arc::new(
                        ...
                        .add_sampled_image(tex_access.image_buffer.clone(),
