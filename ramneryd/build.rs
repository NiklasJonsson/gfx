...
fn main() {
    ...
    if !dest_path.is_dir() {
        if dest_path.exists() {
            std::fs::remove_file(&dest_path).expect(&format!("Failed to remove file (it should be a dir) {}", dest_path.display()));
        }
    ...
    for path in find_ext(&shader_sources, "glsl").into_iter() {
        let dest = dest_path.join(path.strip_prefix(&shader_sources).expect("Failed to relative dir"));
        std::fs::create_dir_all(&dest.parent().expect("No parent")).expect("Failed to created dirs");
