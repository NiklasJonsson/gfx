    ...
    pub fn load(
        ...
        let result: Result<Handle<HostTexture>, image::ImageError> =
            self.storage.get_or_add(asset, |asset| {
                ...
                // SAFETY: We already have raw bytes, it's safe to create a ByteBuffer from them.
                let data = Arc::new(unsafe { ByteBuffer::from_vec(raw_image_data) });
                Ok(HostTexture {
                    data,
                    extent,
                    format: asset.format,
        ...
        fn run(&mut self, data: Self::SystemData) {
            ...
            let map_tex = |tex: HostTextureHandle,
             ...
             -> PendingTextureUse {
                ...
                let data = trekant::DescriptorData::Shared(Arc::clone(&cpu_texture.data));
                let handle = loader
                    .load_texture(
